...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function s() {
    console.log(
      [
        'Are you sure that detectNetwork works for every credit card network given?',
        '',
        "It's pretty normal to feel like your code works because you thought it through carefully when you wrote it. It's also normal to feel frustrated at this process. However, on the job, you won't be able to risk the chance that a function produces unexpected results. The only way to know for sure is to actually run your function for each input, even if it feels tedious.",
        '',
        "To be %cabsolutely certain your function produces expected output all the time%c, you must invoke your function for EVERY combination of length and prefix and make sure it returns the correct network. This must be done in the browser console, and you will not be allowed to advance to the next step until you have done this. If you modify your function, you'll need to refresh the page, and try all of the prefix and length combinations again. This is to ensure your function still works after you made changes.",
        '',
        'For example, for American Express, make sure you try detectNetwork with:',
        '   - A credit card number that starts with 34 and is 15 digits long',
        '   - A credit card number that starts with 37 and is 15 digits long',
        "For Diner's Club, make sure you invoke detectNetwork with:",
        '   - A credit card number that starts with 38 and is 14 digits long',
        '   - A credit card number that starts with 39 and is 14 digits long',
        '',
        'After you have invoked detectNetwork for ALL length and prefix combinations, invoke nextStep:',
        '',
        '%c>%c nextStep()',
      ].join('\n'),
      'font-weight:bold',
  ...
  function c(e) {
    var t = [
        "You didn't achieve %cfull test coverage%c in your Mocha tests! Make sure there is a test for each prefix and length combination for each card.",
        '',
        'For example, for American Express, make sure there is a test with:',
        '   - A credit card number that starts with 34 and is 15 digits long',
        '   - A credit card number that starts with 37 and is 15 digits long',
        '',
      ].join('\n'),
  ...
  function l(e, t) {
    for (var o = { 1: f, 2: d, 3: g, 4: m }, n = !1, r = t; r >= e; r--)
      S(r) &&
        !n &&
        (console.log(
          [
        ...
        e > 2 && i({ mocha: !0 }, function () {}),
        (n = !0),
        o[e]());
    n ||
      console.log(
        [
          "Uh oh, it looks like your function %cisn't working properly for all combinations %cof prefix and length.",
          'Please try again!',
        ].join('\n'),
        'font-weight:bold',
        'font-weight:normal'
  ...
  function u() {
    console.log(
      [
        'STEP ONE:',
        '',
        "Your goal is to write a function that can detect what credit card network a card belongs to based on the card's number. You'll write this function in %cdetectNetwork.js%c, which was included in the repo you downloaded. Go find it now and follow the directions there.",
        '',
        ...
        '',
        "%cWhen you've made detectNetwork return the correct network for every prefix and length combination for Diner's Club and American Express, you can invoke nextStep: %c",
        '',
        '%c>%c nextStep()',
        '',
      ].join('\n'),
  ...
  function h() {
    console.log(
      [
        ...
        '',
        '',
        "%cWhen detectNetwork returns the correct network for every prefix and length combination for Diner's Club, American Express, Visa, and MasterCard you can invoke nextStep: %c",
        '',
        '%c>%c nextStep()',
      ].join('\n'),
      'font-weight:bold',
  ...
  function w() {
    console.log(
      [
        ...
        '',
        "%cWhen you're ready to see the test suite, invoke nextStep.%c",
        '',
        '%c>%c nextStep()',
      ].join('\n'),
      'font-weight:bold',
  ...
  function p() {
    console.log(
      [
        '',
        "Here it is! Each heading (like Diner's Club) is a different set of tests, and underneath each heading is a list of tests, with a ❌ if it fails and a ✔️ if it passes.",
        '',
        'Examine the tests in detectNetwork.test.js until you are confident about the behavior they are testing, then make them pass (you might have to modify a few tests to do so!). Then, implement the following networks:',
        '',
        'Discover always has a prefix of 6011, 644-649, or 65, and a length of 16 or 19.',
        'Maestro always has a prefix of 5018, 5020, 5038, or 6304, and a length of 12-19.',
        '',
        "%cWhen detectNetwork returns the correct network for every prefix and length combination for Diner's Club, American Express, Visa, MasterCard, Discover, and Maestro, you can invoke nextStep: %c",
        '',
        '%c>%c nextStep()',
      ].join('\n'),
      'font-weight:bold',
  ...
  function m() {
    console.log(
      [
        ...
        '',
        "%cWhen detectNetwork returns the correct network for every prefix and length combination for Diner's Club, American Express, Visa, MasterCard, Discover, Maestro, China UnionPay, and Switch you can invoke nextStep: %c",
        '',
        '%c>%c nextStep()',
      ].join('\n'),
      'font-weight:bold',
  ...
  function y() {
    return {
      message: [
        'Writing all of those tests can be repetitive and exhuasting! Loops are a great way to procedurally generate tests.',
        '',
        'This code will help - %cread it carefully, %cand also consider why an IIFE is needed here before moving on.',
        '',
  ...
  function b() {
    console.log(
      [
        'Solid work writing all those tests, but you forgot to refactor your tests to use only %cshould OR expect%c, not both.',
        'Please refactor your tests before proceeding.',
      ].join('\n'),
      'font-weight:bold',
      'font-weight:normal'
    );
  ...
  function k() {
    console.log(
      ['Congratulations! You have finished the exercise!'].join('\n')
    );
  }
  function x() {
  ...
  function N(e, t) {
    ...
    return (
      !(n.length > 0) ||
      (console.log(
        '%cdetectNetwork was NOT invoked with a card number that has the (prefix, length) combination(s):%c \n' +
          n.join('\n') +
          '%c\nSee below for more information.',
        'font-style: italic; background-color: pink;',
        'font-style: italic; background-color: pink; font-weight:bold',
        'font-style: italic; background-color: pink; font-weight:normal'
      ),
      !1)
    );
  }
  ...
  var z,
    ...
    Q = !1,
    X = [
      { print: u, test: j.bind(null, 1), mocha: !1 },
      { print: s, test: o, mocha: !1 },
      { print: l.bind(null, 1, 4), test: t.bind(null, 1, [T, P]), mocha: !1 },
      { print: h, test: j.bind(null, 2), mocha: !1 },
      { print: s, test: n, mocha: !1 },
      { print: l.bind(null, 2, 4), test: t.bind(null, 2, [W, O]), mocha: !1 },
      { print: w, test: j.bind(null, 3), mocha: !1 },
      { print: p, test: D, mocha: !0 },
      { print: c.bind(null, !0), test: r, mocha: !0 },
      { print: b, test: x, mocha: !0 },
      {
        print: l.bind(null, 3, 4),
        test: t.bind(null, 3, [C, Y, L]),
        mocha: !0,
      },
      { print: m, test: j.bind(null, 4), mocha: !0 },
      { print: c.bind(null, !0), test: a, mocha: !0 },
      {
